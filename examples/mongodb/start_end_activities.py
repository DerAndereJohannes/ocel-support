...
def execute_script():
    ...
    ggg = events_collection.aggregate(
        [{"$sort": {"ocel:timestamp": 1}}, {"$unwind": "$ocel:omap"}, {"$group": {"_id": "$ocel:omap", "startactivity": {"$first": "$ocel:activity"}}}, {"$group": {"_id": "$startactivity", "count": {"$count": {}}}}])
    start_activities = Counter({x["_id"]: x["count"] for x in ggg})
    ggg = events_collection.aggregate(
        [{"$sort": {"ocel:timestamp": 1}}, {"$unwind": "$ocel:omap"}, {"$group": {"_id": "$ocel:omap", "endactivity": {"$last": "$ocel:activity"}}}, {"$group": {"_id": "$endactivity", "count": {"$count": {}}}}])
